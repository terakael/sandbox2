...
public class LootResponse extends Response {
	...
	public void process(Request req, Player player, ResponseMaps responseMaps) {
		...
		if (!PathFinder.isNextTo(player.getFloor(), player.getTileId(), request.getTileId())) {
			...
			if (sceneryId == 156) { // necrotic chest
				...
				int graveyardEntranceTileId = Utils.getTileIdFromRelativeXY(
						UndeadArmyManager.getTopLeftTileId(),
						9,
						19);

				// send teleport explosions to both where the player teleported from, and where
				...
				responseMaps.addLocalResponse(graveyardEntranceFloor, graveyardEntranceTileId,
						new TeleportExplosionResponse(graveyardEntranceTileId));
	...
	private void handleLoot(Player player, ResponseMaps responseMaps) {
		...
		if (RandomUtil.chance(10)) {
			...
			PlayerStorageDao.addItemToFirstFreeSlot(player.getId(), StorageTypes.INVENTORY, necroRobeItemId, 1,
					ItemDao.getMaxCharges(necroRobeItemId));
		} else {
			// one of the other treasures
			dropTable.get(RandomUtil.getRandom(0, dropTable.size())).forEach(item -> {
				PlayerStorageDao.addItemToFirstFreeSlot(player.getId(), StorageTypes.INVENTORY, item.getItemId(),
