...
public abstract class WalkAndDoResponse extends Response {
	...
	protected boolean nextToTarget(Request request, Player player, ResponseMaps responseMaps) {
		return PathFinder.isNextTo(player.getFloor(), player.getTileId(),
				target == null ? walkingTargetTileId : target.getTileId());
	}

	protected void walkToTarget(Request request, Player player, ResponseMaps responseMaps) {
		player.setPath(PathFinder.findPath(player.getFloor(), player.getTileId(),
				target == null ? walkingTargetTileId : target.getTileId(), false));
	}
	...
	public void process(Request request, Player player, ResponseMaps responseMaps) {
		...
		// if the player was attacked then we aren't walking anymore.
		if (FightManager.getFightByPlayerId(player.getId()) != null) {
			return;
		}
