...
public class MessageResponse extends Response {
	...
	private void handleDebugCommand(Player player, String msg, ResponseMaps responseMaps) {
		String[] msgParts = msg.split(" (?=(?:[^\"]*\"[^\"]*\")*[^\"]*$)");// the :: prefix should already be removed
																			// here
	...
	private void handleDebugTele(Player player, String[] msgParts, ResponseMaps responseMaps) {
		...
		switch (msgParts[1]) {
			case "tyrotown": {
				TeleportableDto tyrotownTele = TeleportableDao
						.getTeleportableByItemId(Items.TYROTOWN_TELEPORT_RUNE.getValue());
			...
			case "up": {
				floor = player.getFloor() + 1;
				tileId = player.getTileId();
				break;
			}
			case "down": {
				floor = player.getFloor() - 1;
		...
		if (tileId != -1) {
			...
			responseMaps.addLocalResponse(player.getFloor(), player.getTileId(),
					new TeleportExplosionResponse(player.getTileId()));
		...
		if (!PathFinder.tileIsWalkable(player.getFloor(), destTileId)
				&& !PathFinder.tileIsSailable(player.getFloor(), destTileId)) {
	...
	private void handleAddExp(Player player, String[] msgParts, ResponseMaps responseMaps) {
		...
		Player targetPlayer = targetPlayerId == player.getDto().getId() ? player
				: WorldProcessor.getPlayerById(targetPlayerId);

		// inform god that the target player has received the exp
		if (targetPlayer != player) {
			setRecoAndResponseText(1,
					String.format("%s has been granted %dexp in %s, my lord.", msgParts[2], exp, msgParts[0]));
			responseMaps.addClientOnlyResponse(player, this);
		}

		if (targetPlayer == null)
			return;// targetPlayer is valid based on the id check above, but they're not logged in
					// so we can't send them their response
		...
		messageResponse.setRecoAndResponseText(1, String.format("Your god has granted you %dexp in %s; %s him!", exp,
				msgParts[0], exp <= 0 ? "fear" : "praise"));
		...
		if (ItemDao.itemHasAttribute(itemId, ItemAttributes.STACKABLE)) {
			...
			for (int i = 0; i < invItemIds.size() && count > 0; ++i) {
				if (invItemIds.get(i) == 0) {
					PlayerStorageDao.setItemFromPlayerIdAndSlot(player.getId(), StorageTypes.INVENTORY, i, itemId, 1,
							ItemDao.getMaxCharges(itemId));
					--count;
				}
			}
		}
	...
	private void handleHeal(Player player, String[] msgParts, ResponseMaps responseMaps) {
		...
		if (targetPlayer != player) {
			...
			godResponse.setRecoAndResponseText(1,
					String.format("you have given %s new life, my lord.", targetPlayer.getDto().getName()));
