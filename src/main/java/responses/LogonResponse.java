...
public class LogonResponse extends Response {
	...
	public void processLogon(Request req, Session client, ResponseMaps responseMaps) {
		...
		// TODO this should be done when we create the player, but the creation process
		// doesn't exist yet
		...
		// if there was a bad disconnection (server crash etc) and the player was
		// mid-trade, put the items back into the player's inventory.
		Map<Integer, InventoryItemDto> itemsInTrade = PlayerStorageDao.getStorageDtoMapByPlayerId(playerDto.getId(),
				StorageTypes.TRADE);
		for (InventoryItemDto itemInTrade : itemsInTrade.values().stream().filter(e -> e.getItemId() != 0)
				.collect(Collectors.toList())) {
			PlayerStorageDao.addItemToFirstFreeSlot(playerDto.getId(), StorageTypes.INVENTORY, itemInTrade.getItemId(),
					itemInTrade.getCount(), itemInTrade.getCharges());
		}
		...
		// if the player is underground or it's currently night time, then let the
		// client know to set the night filter.
