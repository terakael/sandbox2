...
import processing.attackable.bosses.NecromancerFirstForm;
import processing.attackable.bosses.NecromancerSecondForm;
...
public class UndeadArmyManager {
	...
	private static final int customAreaId = 23;
	...
	private static int graveyardCentreTileId;
	@Getter
	private static int topLeftTileId;
	private static int bottomRightTileId;
	private static int cryptTopLeftTileId;
	private static int cryptBottomRightTileId;
	...
	private static Set<Integer> undeadEntLocations;

	public static void init() {
		DbConnection.load("select tile_id from custom_area where id=?",
				rs -> topLeftTileId = rs.getInt("tile_id"),
				customAreaId);

		UndeadArmyWavesDao.setupCaches(topLeftTileId);

		DbConnection.load(
				"select max(offset_x) as offset_x, max(offset_y) as offset_y from custom_room_ground_textures crgt where custom_area_id=?",
				rs -> {
					bottomRightTileId = topLeftTileId + rs.getInt("offset_x")
							+ (rs.getInt("offset_y") * PathFinder.LENGTH);
					graveyardCentreTileId = Utils.getTileIdFromRelativeXY(topLeftTileId, rs.getInt("offset_x") / 2,
							(rs.getInt("offset_y") / 2) - 1);
				},
				customAreaId);

		undeadEntLocations = new HashSet<>();
		DbConnection.load("select offset_x, offset_y from custom_room_scenery where custom_area_id=? and scenery_id=?",
				rs -> undeadEntLocations.add(
						Utils.getTileIdFromRelativeXY(topLeftTileId, rs.getInt("offset_x"), rs.getInt("offset_y"))),
				customAreaId, entSceneryId);

		// hacky, but we'll find the crypt bounds by taking the locked door and
		// hardcoding relative to that.
		DbConnection.load("select offset_x, offset_y from custom_locked_door_instances where custom_area_id=?",
				rs -> {
					int doorOffsetX = rs.getInt("offset_x");
					int doorOffsetY = rs.getInt("offset_y");
					cryptTopLeftTileId = topLeftTileId + (doorOffsetX - 1) + ((doorOffsetY - 2) * PathFinder.LENGTH);
					cryptBottomRightTileId = topLeftTileId + (doorOffsetX + 1) + (doorOffsetY * PathFinder.LENGTH);
				}, customAreaId);
	...
	public static void initNpcs() {
		...
		allWaveNpcs.get(entWave - 1)
				.add(new NecromancerFirstForm(NPCDao.getNpcById(firstFormNecromancerNpcId), 0, graveyardCentreTileId));
		...
		allWaveNpcs.putIfAbsent(entWave + 1, new ArrayList<>());
		allWaveNpcs.get(entWave + 1).add(
				new NecromancerSecondForm(NPCDao.getNpcById(secondFormNecromancerNpcId), 0, graveyardCentreTileId));
	}
	...
	public static void checkWaveStatus(ResponseMaps responseMaps) {
		...
		if (currentWaveNpcs.values().stream().filter(e -> e.getCurrentHp() > 0).findAny().isEmpty()) {
			if (currentWave == entWave) {
				...
				responseMaps.addLocalResponse(0, graveyardCentreTileId,
						MessageResponse.newMessageResponse("necromancer: you killed all my trees...", "yellow"));
			} else
				newWave(responseMaps);
		}
	}
	...
	private static void newWave(ResponseMaps responseMaps) {
		...
		MessageResponse messageResponse = MessageResponse
				.newMessageResponse(String.format("starting wave %d.", currentWave), "white");
		...
		if (currentWave == entWave) {
			...
			undeadEntLocations.forEach(tileId -> {
				// when the trees aren't scenery, players can walk on the tile.
				PathFinder.setImpassabilityOnTileId(0, tileId, 0);
				SceneryDao.removeSceneryFromTileId(0, tileId, entSceneryId);
			...
			final List<Player> localPlayers = getPlayersInGraveyard().stream()
					.filter(player -> !FightManager.fightWithFighterExists(player)) // exclude any players currently
																					// fighting (could be duelling or
																					// whatevs)
					.collect(Collectors.toList());

			// if we choose a player inside the crypt room, then the necromancer will
			// teleport inside, then nobody will be able to attack him if they don't have
			// the key
			final List<Player> playersOutsideCryptRoom = localPlayers.stream()
					.filter(player -> !Utils.tileIdWithinRect(player.getTileId(), cryptTopLeftTileId,
							cryptBottomRightTileId))
					.collect(Collectors.toList());
			...
			final int tileId = targetPlayer == null ? graveyardCentreTileId : targetPlayer.getTileId(); // idk here i
																										// guess
			...
			responseMaps.addLocalResponse(0, tileId,
					MessageResponse.newMessageResponse(String.format("necromancer: %s", message), "yellow"));
	...
	private static void resetEnts(ResponseMaps responseMaps) {
		undeadEntLocations.forEach(tileId -> {
			...
			// 1. the ents were awoken during the night, and therefore their impassability
			// was changed to 0.
			// in this situation, we reset the impassability to 15. We also add the scenery
			// back for players
			// that are near the trees, as scenery is only sent to the client when they come
			// into range
			// (i.e. they won't update until the player leaves the area and then comes back
			// into range otherwise).
			// 2. the ents are not awoken, and the trees remain as they are.
			// in this case, the impassability will already be 15, meaning we don't need to
			// do anything.

			SceneryDao.insertSceneryAtTileId(0, tileId, entSceneryId);

			if (PathFinder.getImpassableByTileId(0, tileId) == 0) {
				PathFinder.setImpassabilityOnTileId(0, tileId, 15); // completely impassable
				// there might be some players that have not loaded a dead tree that arrived as
				// the trees were ents
				LocationManager.getLocalPlayers(0, tileId, 12).forEach(player -> {
					...
					inRangeResponse
							.setInstances(Map.<Integer, Set<Integer>>of(entSceneryId, Collections.singleton(tileId)));

					// whenever we update the scenery the doors/depleted scenery are reset, so we
					// need to reset them.
	...
	public static int getSceneryIdByTileId(int floor, int tileId) {
		...
		// as soon as daybreak arrives, the currentWave is reset to 0 so the trees will
		// show.
	...
	public static Set<Player> getPlayersInGraveyard() {
		return LocationManager.getLocalPlayersWithinRect(0, topLeftTileId, bottomRightTileId);
	}
	...
	public static void addPlayerGrownZombie(Player planter, int floor, int tileId) {
		...
		deepCopy.setAttributes(deepCopy.getAttributes() & ~NpcAttributes.AGGRESSIVE.getValue()); // they should be
																									// unaggressive
																									// otherwise mad
																									// griefing ensues
		deepCopy.setAttributes(deepCopy.getAttributes() | NpcAttributes.DIURNAL.getValue()); // should show at all times
																								// of the day
		deepCopy.setAttributes(deepCopy.getAttributes() | NpcAttributes.NOCTURNAL.getValue());
		deepCopy.setRespawnTicks(5); // onRespawn is where the zombie is removed from the game (onDeath is too early
										// as we wanna see the death animation)
