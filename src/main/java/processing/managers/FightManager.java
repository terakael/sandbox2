...
import responses.PvmEndResponse;
import responses.PvpEndResponse;
...
public class FightManager {
	...
	public static class Fight {
		@Getter
		private Attackable fighter1;
		@Getter
		private Attackable fighter2;
		@Getter
		...
		Fight(Attackable fighter1, Attackable fighter2, boolean fighter1initiated, Integer rules) {
			...
			if (rules != null && (rules & DuelRules.no_retreat.getValue()) > 0)
				battleLockTicks = Integer.MAX_VALUE; // technically it's not "no retreat": you can retreat after ~40
														// years.
		...
		public void process(ResponseMaps responseMaps) {
			...
			if (WorldProcessor.getPlayerById(((Player) fighter1).getId()) == null) {
				...
				return;
					...
					&& WorldProcessor.getPlayerById(((Player) fighter2).getId()) == null) {
				cancelFight(fighter2, responseMaps);
				return;
			}

			// if the players aren't on the same tile then they are still closing in on
			// eachother
	...
	public static void process(ResponseMaps responseMaps) {
		// we want to copy all the fights into a different container, as if a fight ends
		// during processing
	...
	public static void addFightWithRules(Attackable fighter1, Attackable fighter2, boolean fighter1initiated,
			Integer rules) {
	...
	public static void cancelFight(Attackable participant, ResponseMaps responseMaps) {
		for (Fight fight : fights) {
			if (fight.getFighter1() == participant || fight.getFighter2() == participant) {
				...
				// if we're cancelling a fight with an npc, the npc should keep the target so it
				// can chase.
				...
				if (fight.getFighter2() instanceof NPC) {
					...
					responseMaps.addLocalResponse(((Player) fight.getFighter1()).getFloor(),
							((Player) fight.getFighter1()).getTileId(), resp);
					...
					responseMaps.addLocalResponse(((Player) fight.getFighter2()).getFloor(),
							((Player) fight.getFighter2()).getTileId(), resp);

					fight.getFighter2().setTarget(null); // only unset the target if it's a player. NPCs will go after
															// the retreating player again.
				}
