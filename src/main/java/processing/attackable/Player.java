...
public class Player extends Attackable {
	public enum PlayerState {
		idle,
		walking,
		chasing, // used for walking to a moving target (following, moving to attack something
					// etc)
		chasing_with_range, // if the player is behind a wall and they cast a spell, they'll run around in
							// preparation of casting from range
		mining,
		smithing,
		smelting,
		woodcutting,
		...
		fighting,
		using,
		climbing, // ladders etc, give a tick or so duration (like for climbing animation in the
					// future)
		cooking,
		fishing,
		growing_zombie,
	...
	@Setter
	@Getter
	private NpcDialogueDto currentDialogue = null;
	@Setter
	@Getter
	private int shopId;// if the player is currently in a shop, this is the id
	...
	@Getter
	@Setter
	private Set<Integer> inRangeNpcs = new HashSet<>();
	@Getter
	@Setter
	private Set<Integer> inRangePlayers = new HashSet<>();
	@Getter
	@Setter
	private Set<Integer> inRangeConstructables = new HashSet<>();
	@Getter
	@Setter
	private Map<Integer, List<Integer>> inRangeGroundItems = new HashMap<>();
	@Getter
	@Setter
	private Set<Integer> inRangeShips = new HashSet<>();
	@Getter
	@Setter
	private Set<Integer> localTiles = new HashSet<>();
	@Getter
	@Setter
	private int loadedFloor = 0;
	@Getter
	@Setter
	private Set<Integer> loadedMinimapSegments = new HashSet<>();
	@Getter
	private Set<Integer> activePrayers = new HashSet<>();
	@Getter
	private float prayerPoints = 0;
	@Setter
	private int range = 0; // if we're chasing with range (i.e. chasing to cast a spell or something), this
							// is the range we want to be in
	private boolean slowburnBlockedStatDrain = false;
	private int tybaltsCapeTimer = 0; // tybalts cape gives 1hp per minute of wearing it (essentially doubling hp
										// regen)
	...
	// technically they are at the respawn point, and if they disconnect during the
	// death animation
	// then that's where they'll log in. However during the death animation we still
	// want the client
	// to receive messages at the point of their death (to continue processing npcs,
	// scenery etc until they respawn).
	...
	public Player(PlayerDto dto, Session session) {
		...
		if (dto != null) {
			...
			prayerPoints = StatsDao.getStatLevelByStatIdPlayerId(Stats.PRAYER, dto.getId())
					+ StatsDao.getRelativeBoostsByPlayerId(dto.getId()).get(Stats.PRAYER);
			currentHp = StatsDao.getStatLevelByStatIdPlayerId(Stats.HITPOINTS, dto.getId())
					+ StatsDao.getRelativeBoostsByPlayerId(dto.getId()).get(Stats.HITPOINTS);
	...
	public void process(int tick, ResponseMaps responseMaps) {
		...
		if (EquipmentDao.isItemEquipped(getId(), 359)) { // tybalt's cape
			if (++tybaltsCapeTimer >= 100) { // once-per-minute hp regen; effectively doubling hp regen (still stackable
												// with prayers etc)
				tybaltsCapeTimer = 0;
				incrementHitpoints(responseMaps);
			}
		} else {
			if (tybaltsCapeTimer > 0) // to stop the ability to flick the cape, the timer resets to 0 if the cape is
										// unequipped
				tybaltsCapeTimer = 0;
		}
		...
		switch (state) {
			case walking: {
				// popPath returns true if we've successfully moved to the next tile; false if a
				// door is in the way or the path is empty
				if (popPath(responseMaps)) {
					...
					// we want to clear the saved request beforehand so we don't overwrite it if it
					// sets it,
			...
			case chasing: {
				if (target != null && floor != target.getFloor())// if the target goes down a ladder or something then
																	// stop following
					target = null;
				...
				// similar to walking, but need to recalculate path each tick due to moving
				// target
				if (popPath(responseMaps)) {
					PlayerUpdateResponse playerUpdateResponse = new PlayerUpdateResponse();
					playerUpdateResponse.setId(dto.getId());
			...
			case mining:
				// waiting until the tick counter hits zero, then do the actual mining and
				// create a finish_mining response
				if (--tickCounter <= 0) {
			...
			case construction:
				if (--tickCounter <= 0) {
					...
					if (state == PlayerState.idle) // if we're flatpacking and the workbench disappears then the player
													// state gets set to idle.
						break;
			...
			case disassembling:
				if (--tickCounter <= 0) {
					...
					responseMaps.addClientOnlyResponse(this,
							MessageResponse.newMessageResponse("...it shatters to pieces.", "white"));
					savedRequest = null;
					setState(PlayerState.idle);
				}
				break;
			...
			case cooking:
				if (--tickCounter <= 0) {
					...
					// finishCooking response can set the state back to idle if the fire runs out
					// during the cook.
					// in this case, don't try and do another UseResponse as it'll result in an
					// annoying "nothing interesting happens".
			...
			case dead: {// note that while the player is in the dead state, every response (except
						// Message) is ignored at the Response superclass level
				if (--tickCounter <= 0) {
					state = PlayerState.idle;
					...
					// (they also receive the player_in_range response automatically but if they're
					// around the spawn point it will be missing respawn data)
	...
	private void processPrayer(ResponseMaps responseMaps) {
		float prayerBonus = 1 - ((float) Math.min(49, bonuses.get(Stats.PRAYER)) / 50); // each prayer point is 2% pray
																						// reduction
	...
	public void incrementHitpoints(ResponseMaps responseMaps) {
		...
		if (newRelativeBoost > getBonuses().get(Stats.HITPOINTS))// the hp relative boost is negative for how many hp
																	// lost
	...
	private void restoreStats(ResponseMaps responseMaps) {
		...
		for (Map.Entry<Stats, Integer> entry : boosts.entrySet()) {
			...
			// max hitpoints depends on the hitpoints bonus (i.e. +5 means we can heal +5
			// over max hitpoints)
	...
	public void setTileId(int tileId) {
		// TODO if we're on a boat and we tele off, we should disembark (unless its the
		// boat setting the tileId)
	...
	public void setFloor(int floor, ResponseMaps responseMaps) {
		// if we go underground, or come up from underground, reset the brightness
		// accordingly.
	...
	public void handleWalkingThroughClosedDoor(int doorTileId, ResponseMaps responseMaps) {
		...
		// TODO this is kinda shitty because we're repeating some of the
		// OpenCloseResponse logic...
		final LockedDoorDto lockedDoor = LockedDoorManager.getLockedDoor(getFloor(), doorTileId);

		// order here is important - OpenCloseResponse can destroy an item which is used
		// in the requirements check
		final boolean canGoThroughDoor = lockedDoor == null
				|| LockedDoorManager.playerMeetsDoorRequirements(this, lockedDoor).isEmpty();
		...
		if (!canGoThroughDoor) {
			...
			// in the case of following a player, it tries to walk through over and over
			// every tick.
			// i guess if we hit a locked door then the player should give up and stop
			// following?
			setState(PlayerState.idle);
		}
	}
	...
	public void onDeath(Attackable killer, ResponseMaps responseMaps) {
		// so the client continues to receive messages and process correctly during
		...
		Fight fight = FightManager.getFightByPlayerId(getId()); // fight doesn't necessarily exist - the player could
																// die of poison outside a fight for example.
		...
		if (duelRules == null || (duelRules & DuelRules.dangerous.getValue()) > 0) {
			...
			Map<Integer, InventoryItemDto> inventoryList = PlayerStorageDao.getStorageDtoMapByPlayerId(getId(),
			...
			for (InventoryItemDto dto : inventoryList.values()) {
				...
				if (dto.getItemId() != 0 && PetDao.getNpcIdFromItemId(dto.getItemId()) == -1) {
					int stack = ItemDao.itemHasAttribute(dto.getItemId(), ItemAttributes.STACKABLE) ? dto.getCount()
							: 1;
					int charges = ItemDao.itemHasAttribute(dto.getItemId(), ItemAttributes.CHARGED) ? dto.getCharges()
							: 1;

					if (killer instanceof Player
							&& ItemDao.itemHasAttribute(dto.getItemId(), ItemAttributes.TRADEABLE)) {
						// if it's a tradeable unique, the killer should only see it if they don't
						// already have one.
						if (ItemDao.itemHasAttribute(dto.getItemId(), ItemAttributes.UNIQUE)) {
							if (PlayerStorageDao.itemExistsInPlayerStorage(((Player) killer).getId(), dto.getItemId())
									|| GroundItemManager.itemIsOnGround(((Player) killer).getId(), dto.getItemId())) {
								continue;
							}
						}

						GroundItemManager.add(getFloor(), ((Player) killer).getId(), dto.getItemId(), getTileId(),
								stack, charges);
					} else {
		...
		if (duelRules != null) { // not an "else" here because we still need to handle the dangerous duel stake
									// stuff (which is irrelevant to do but still possible)
			// safe/dangerous duel: pull stuff from own trade (staked stuff) and throw it on
			// killer's ground
			// no need to do the tradeable checks etc; this is all checked pre-duel when the
			// duel offer is being done
			List<InventoryItemDto> itemsToDrop = PlayerStorageDao
					.getStorageDtoMapByPlayerId(getId(), StorageTypes.TRADE).values().stream()
					.filter(e -> e.getItemId() != 0)
					.collect(Collectors.toList());

			for (InventoryItemDto itemToDrop : itemsToDrop) {
				GroundItemManager.add(getFloor(), ((Player) killer).getId(), itemToDrop.getItemId(), getTileId(),
						itemToDrop.getCount(), itemToDrop.getCharges());
			}
		...
		// if the player's spawn point is not local to where they died, they won't
		// receive the local death response.
		// it's not ideal to send an additional response but we have to set the respawn
		// point immediately
		// otherwise the player can just disconnect during the death sequence then
		// relogin in the same spot they died (with full hp)
		// if (respawnPoint.getFloor() != getFloor() ||
		// !Utils.areTileIdsWithinRadius(respawnPoint.getTileId(), getTileId(), 15))
	...
	public void onKill(Attackable killed, ResponseMaps responseMaps) {
		...
		if (killed instanceof Player) {
			...
			messageResponse.setRecoAndResponseText(1,
					String.format("you have defeated %s!", ((Player) killed).getDto().getName()));
			messageResponse.setColour("white");
			responseMaps.addClientOnlyResponse(this, messageResponse);

			Fight fight = FightManager.getFightByPlayerId(getId()); // fight doesn't necessarily exist - the player
																	// could die of poison outside a fight for example.
			Integer duelRules = fight == null ? null : fight.getRules();
			if (duelRules != null) {
				// they won this duel (clearly); put their stake items back into their inventory
				Map<Integer, InventoryItemDto> stakedItems = PlayerStorageDao.getStorageDtoMapByPlayerId(getId(),
						StorageTypes.TRADE);
				for (InventoryItemDto item : stakedItems.values())
					PlayerStorageDao.addItemToFirstFreeSlot(getId(), StorageTypes.INVENTORY, item.getItemId(),
							item.getCount(), item.getCharges());
				PlayerStorageDao.clearStorageByPlayerIdStorageTypeId(getId(), StorageTypes.TRADE);

				InventoryUpdateResponse.sendUpdate(this, responseMaps);
			}
		} else {
			TybaltsTaskManager.check(this,
					new KillNpcTaskUpdate(((NPC) killed).getId(), killed.getFloor(), killed.getTileId()), responseMaps);
		}

		// exp is doled out based on attackStyle and weapon type.
		// exp is split into five parts (called points) and the points are stored as
		// follows:
	...
	public void onHit(int damage, DamageTypes type, ResponseMaps responseMaps) {
		...
		if (activeBuffs.containsKey(Buffs.RESTORATION) && type != DamageTypes.POISON)// don't kill the buff if it's due
																						// to poison
		...
		if (type == DamageTypes.POISON && prayerIsActive(Prayers.FAITH_HEALING)) {
			...
			if (damageToPrayer > 0) {
				...
				// drain the remainder of the prayer points, remainder of the damage still gets
				// hit
				...
				if ((opponent instanceof NPC)) {
					if (NPCDao.npcHasAttribute(((NPC) opponent).getId(), NpcAttributes.VENOMOUS)) {
						if (RandomUtil.chance(20)) {
							inflictPoison(4);
							responseMaps.addClientOnlyResponse(this,
									MessageResponse.newMessageResponse("you have been poisoned!", "#00aa00"));
						}
					}
				}
			}
	...
	private int handleReinforcedItemDegradation(int damage, ResponseMaps responseMaps) {
		...
		// platebody: 15%
		// shield: 20%
		// each piece is chosen randomly to soak, and multiple pieces can be chosen on
		// the same hit.
		...
		Map<Integer, Integer> equippedSlotsCopy = equippedSlotsByItemId.entrySet().stream()
				.collect(Collectors.toMap(e -> e.getKey(), e -> e.getValue()));

		boolean itemUpdated = false;
		boolean itemCleared = false;
		for (Map.Entry<Integer, Integer> entry : equippedSlotsCopy.entrySet()) {
			ReinforcementBonusesDto reinforcementBonuses = ReinforcementBonusesDao
					.getReinforcementBonusesById(entry.getKey());
			...
			if (damage > 0) {
				if (RandomUtil.getRandom(0, 100) < reinforcementBonuses.getProcChance()) {
					...
					InventoryItemDto item = PlayerStorageDao.getStorageItemFromPlayerIdAndSlot(getId(),
							StorageTypes.INVENTORY, entry.getValue());
					...
					if (item.getCharges() > damageToSoak) {
						...
						PlayerStorageDao.setItemFromPlayerIdAndSlot(getId(), StorageTypes.INVENTORY, entry.getValue(),
								degradedItemId, 1, ItemDao.getMaxCharges(degradedItemId));
						...
						responseMaps.addClientOnlyResponse(this,
								MessageResponse.newMessageResponse(String.format("Your %s degraded!",
										ItemDao.getNameFromId(item.getItemId(), false)), "white"));
					}
					itemUpdated = true;
				}
			}
	...
	public boolean isItemEquipped(Items item) {
		return equippedSlotsByItemId.containsKey(item.getValue());
	}
	...
	public void setPrayerPoints(float newPrayerPoints, ResponseMaps responseMaps) {
		...
		StatsDao.setRelativeBoostByPlayerIdStatId(getId(), Stats.PRAYER,
				-StatsDao.getStatLevelByStatIdPlayerId(Stats.PRAYER, getId()) + (int) prayerPoints);
	...
	public void setPet(Integer petItemId) {
		if (petItemId == null) {
			...
			// It still exists for a bit of time in the LocationManager, and is able to
			// wander around for a minute or so,
		...
		deepCopy.setAttributes(deepCopy.getAttributes() & ~NpcAttributes.ATTACKABLE.getValue()); // pets cannot be
																									// attacked
		deepCopy.setAttributes(
				deepCopy.getAttributes() | NpcAttributes.DIURNAL.getValue() | NpcAttributes.NOCTURNAL.getValue()); // should
																													// show
																													// at
																													// all
																													// times
