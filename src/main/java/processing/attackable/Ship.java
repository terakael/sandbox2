...
public class Ship extends Attackable {
	@Getter
	private final int captainId; // player who owns the ship
	@Getter
	...
	private void processRepairs(ResponseMaps responseMaps) {
		if (passengerWithState(PlayerState.repairing_ship) == null) {
			...
			final String noSuppliesMessage = String.format("there's no %s available in storage to repair the ship.",
					ItemDao.getItem(repairPlankId).getNamePlural());
					...
					.forEach(e -> {
						if (currentRepairTicks.putIfAbsent(e.getId(), 0) != null) {
							...
							if (storage.contains(repairPlankId)) {
								responseMaps.addLocalResponse(floor, tileId,
										new ActionBubbleResponse(this, e.getId(),
												ItemDao.getItem(Items.HAMMER.getValue())));
							} else {
								responseMaps.addClientOnlyResponse(e,
										MessageResponse.newMessageResponse(noSuppliesMessage, "white"));
						...
						if (currentRepairTicks.remove(e.getId(), repairTicks)) {
							...
							if (firstPlank != null) {
								responseMaps.addClientOnlyResponse(e,
										MessageResponse.newMessageResponse("you make some repairs.", "white"));
					...
					.forEach(e -> {
						if (currentFishingTicks.putIfAbsent(e.getId(), 0) != null) {
							...
							if (storage.getEmptySlotCount() > 0) {
								responseMaps.addLocalResponse(floor, tileId,
										new ActionBubbleResponse(this, e.getId(),
												ItemDao.getItem(Items.NET.getValue())));
							} else {
								responseMaps.addClientOnlyResponse(e,
										MessageResponse.newMessageResponse("vessel storage is full.", "white"));
								e.setState(PlayerState.idle);
							}
						}
						...
						if (currentFishingTicks.remove(e.getId(), fishingTicks)) {
							...
							for (int i = 0; i < fishingPoints; ++i) {
								if (storage.getEmptySlotCount() > 0) {
									...
									if (depth >= 30) {
										...
										final boolean nearBabyColossal = OceanFishingManager
												.isNearBabyColossalTile(floor, tileId);
										if (RandomUtil.chance(50 + (Boolean.compare(nearBabyColossal, false) * 15))) {
											...
											if (!nearBabyColossal) {
												...
												// we just caught a giant squid while already in the baby colossal zone
												// - small chance it's a baby colossal
							...
							if (caughtItems.size() == 1) {
								catchMessage = String.format("you catch a %s.",
										ItemDao.getNameFromId(caughtItems.get(0), false));
								...
								catchMessage = String.format("you catch %s.", caughtItems.stream()
										.collect(Collectors.groupingBy(
												...
												Collectors.counting()))
										.entrySet().stream()
										.map(entry -> String.format("%dx %s", entry.getValue(),
												ItemDao.getNameFromId(entry.getKey(), entry.getValue() != 1)))
										.collect(Collectors.joining(", ")));

							responseMaps.addClientOnlyResponse(e,
									MessageResponse.newMessageResponse(catchMessage, "white"));
	...
	public void process(int tick, ResponseMaps responseMaps) {
		...
		ShipUiUpdateResponse.builder()
				.disembarkedPlayers(loggedOutPassengers.stream().map(player -> player.getDto().getName())
	...
	private void fireCannon(ResponseMaps responseMaps) {
		...
		// accuracy is basically 100% when adjacent, then linearly fading to 10% at
		// getCannonRange()
		final double accuracy = 1 - (distanceToTarget / (double) getCannonRange());

		// based on the distance, we can also calculate how many ticks to wait before
		// damaging the target
		...
		LocationManager.getLocalPlayers(floor, tileId, 12)
				.forEach(localPlayer -> ClientResourceManager.addSpriteFramesAndSpriteMaps(localPlayer,
						Collections.singleton(firstCannonballSpriteFrameId)));

		final CastSpellResponse projectile = new CastSpellResponse(tileId, target.getInstanceId(), target.getType(),
				firstCannonballSpriteFrameId, 0.25, 2);
		responseMaps.addLocalResponse(floor, tileId, projectile);

		// TODO hit should take opponent defense into account
		int damageCap = (getCannonRange() - (int) distanceToTarget) * 3;
		if (firstCannonball.getItemId() == Items.GOLDEN_CANNONBALL.getValue())
			damageCap += 7;
		target.onHit(RandomUtil.getRandom(0, damageCap), DamageTypes.STANDARD, responseMaps);
