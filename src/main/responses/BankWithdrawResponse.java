...
public class BankWithdrawResponse extends Response {
	...
	public void process(Request req, Player player, ResponseMaps responseMaps) {
		...
		InventoryItemDto bankItemDto = PlayerStorageDao.getStorageItemFromPlayerIdAndSlot(player.getId(), StorageTypes.BANK.getValue(), request.getSlot());
		if (bankItemDto == null) {
			// cannot withdraw this item (because it doesn't exist).
			return;
		}
		
		ArrayList<Integer> inventoryItems = PlayerStorageDao.getStorageListByPlayerId(player.getId(), StorageTypes.INVENTORY.getValue());
		...
		if (ItemDao.itemHasAttribute(bankItemDto.getItemId(), ItemAttributes.STACKABLE)) {
			...
			PlayerStorageDao.addItemToFirstFreeSlot(player.getId(), StorageTypes.INVENTORY.getValue(), bankItemDto.getItemId(), actualCount, bankItemDto.getCharges());
		} else {
			...
			for (int slot = 0; slot < inventoryItems.size(); ++slot) {
				...
				PlayerStorageDao.addItemToFirstFreeSlot(player.getId(), StorageTypes.INVENTORY.getValue(), bankItemDto.getItemId(), 1, bankItemDto.getCharges());
		...
		if (actualCount >= bankItemDto.getCount()) {
			...
			PlayerStorageDao.setItemFromPlayerIdAndSlot(player.getId(), StorageTypes.BANK.getValue(), request.getSlot(), 0, 1, 0);
			...
			PlayerStorageDao.addCountToStorageItemSlot(player.getId(), StorageTypes.BANK.getValue(), request.getSlot(), -actualCount);
		}
